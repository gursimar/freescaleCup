...
#define LB_WIDTH 13	//10
#define UB_WIDTH 30	//30
#define FILTER 50
...
uint8_t kp = 5.8;
int16_t diff=0;
uint32_t int_time=20000; 
uint16_t dc_speed_a = 400;
...
void initEMIOS_0ch4(void) {        		/* EMIOS 0 CH 4: Output Pulse Width Modulation*/
	...
	EMIOS_0.CH[4].CBDR.R = 1600;      	/* Trailing edge when channel counter bus=1400 Middle, 1650 Right Max, 1150 Left Max*/
...
// 6. MAIN STATE MACHINE ALGO		
       ...
       //FILTER
		if (((centre - prev_centre) > FILTER) || ((centre-prev_centre) < -FILTER))
		{
			prev_centre = centre;
			centre = centre * 0.20;
			//SIU.PGPDO[2].R &= 0x0b000000;		// Enable LED2
...
// 8. Calculate error
		...
		{
			dc_speed_left(dc_speed_a+100);
			dc_speed_right(dc_speed_a+100);
			//SIU.PGPDO[2].R |= 0x08000000;		// Disable LED1
		}

		else if (error < 25 && error >-25)
		{
			dc_speed_left(dc_speed_a+50);
			dc_speed_right(dc_speed_a+50);
			//SIU.PGPDO[2].R &= 0x07000000;		// Enable LED1	
		}
		else
		{
			// some modification is possible that can make speed 0 for some time after sprint..
			dc_speed_left(dc_speed_a);
			dc_speed_right(dc_speed_a);
			//SIU.PGPDO[2].R |= 0x08000000;		// Disable LED1
		}
