...
volatile int16_t diff_result[127];             	/* Read converstion result from ADC input ANS0 */
...
void initEMIOS_0ch6(void) {        		/* EMIOS 0 CH 6: Output Pulse Width Modulation*/
	...
	EMIOS_0.CH[6].CBDR.R = 400;     	/* Trailing edge when channel counter bus=500*/
...
void initEMIOS_0ch7(void) {        		/* EMIOS 0 CH 7: Output Pulse Width Modulation*/
	...
	EMIOS_0.CH[7].CBDR.R = 400;     	/* Trailing edge when channel's counter bus=999*/
...
// 6. Feed the new value to servo motor
