...
#define LB_WIDTH 37
#define UB_WIDTH 55
#define MIN_FINGER 25
#define FINGER 10
#define RIGHT_GUARD 73
#define LEFT_GUARD 48


//Prototypes
void TransmitCharacter(uint8_t ch);
void FLUSH_data_simar(void);
void CAMERA_simar(void);
...
uint8_t kp = 5.8;
int16_t diff=0;
uint32_t int_time=200000; 

// Main loop variables
int16_t max_val, min_val;
uint8_t max_I, min_I, centre=64;
int8_t error;
int16_t pid_term;
uint16_t correction;
...
void initEMIOS_0ch7(void) {        		/* EMIOS 0 CH 7: Output Pulse Width Modulation*/
	...
	EMIOS_0.CH[7].CBDR.R = 300;     	/* Trailing edge when channel's counter bus=999*/
...
// 4. Find maximum and minimum indices
		...
		for (i=centre-1;i>0;i--)	//0 skipped
		{
...
// 6. MAIN STATE MACHINE ALGO
       ...
       else
       {
            ...
            if (max_I > 12 && max_I < 117)
                diff = Result[max_I+FINGER] - Result[max_I-FINGER];
            else if (max_I <= 12)
                diff = Result[max_I+FINGER] - Result[2];
            else if (max_I>116)
                diff = Result[127] - Result[max_I-FINGER];
            ...
            else
            {
	            ...
	            if (min_I > 12 && min_I < 117)
    	            diff = Result[min_I-FINGER] - Result[min_I+FINGER];
        	    else if (min_I <= 12)
            	    diff = Result[2] - Result[min_I+FINGER];
            	else if (max_I>116)
                	diff = Result[max_I-FINGER] - Result[127];
