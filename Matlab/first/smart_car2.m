...
%    there must be atleast one pseudo in this case. As width is not right.
            ...
            if (M > 10 && M < 118)
                DIFF = sen_val(M+FINGER) - sen_val(M-FINGER);
            elseif (M < 11)
                DIFF = sen_val(M+FINGER) - sen_val(2);
            elseif (M>117)
                DIFF = sen_val(128) - sen_val(M-FINGER);
            end
            ...
            if( DIFF > MIN_FINGER) % M is not pseudo
                ...
                if (m > 10 && m < 118)
                    DIFF = sen_val(m-FINGER) - sen_val(m+FINGER);
                elseif (m < 11)
                    DIFF = sen_val(2) - sen_val(m+FINGER);
                elseif (m>117)
                    DIFF = sen_val(m-FINGER) - sen_val(128);
                end
